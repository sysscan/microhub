--[[ Warfare ù module assembly + lifecycle ]]
...
function M.run()
	warn("[Warfare] build", Constants.GAME_BUILD)

	local LocalPlayer = Players.LocalPlayer
	local Camera = workspace.CurrentCamera

	local sharedState = { killAllForcedTarget = nil }
	local hitRate = HitRateLib.create({
		config = Config,
		constants = Constants,
		shared = sharedState,
	})

	local acDbg = AcDebugLib.create({
		config = Config,
...
local function getTargetColor(target)
	if target.player then
		return hitRate.getTeamColor(getTeamRelation(target.player))
	end
	if target.drone then
		local teamName = target.drone:GetAttribute("Team")
		return hitRate.getTeamColor(getInstanceTeamRelation(target.drone, teamName and tostring(teamName) or nil))
	end
...
if typeof(UILib) ~= "table" or typeof(UILib.create) ~= "function" then
	error("MicroHub UI library not loaded ù run hub/loader.lua", 0)
end
			...
			sections = {
				{
					...
					items = {
						...
						{ type = "label", text = "ESP colors ù tap swatch" },
...
local function updateFlight()
	...
	local speed = Constants.FLIGHT_SPEED
	if UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) or UserInputService:IsKeyDown(Enum.KeyCode.RightShift) then
		speed = Constants.FLIGHT_BOOST_SPEED
	end
...
local function drawPlayerEsp(player, character, humanoid, root, camPos, snapFrom)
	...
	local accent = hitRate.getTeamColor(relation)
	local cx = x + w * 0.5
	local bottom = y + h
...
local function updateDroneESP()
	...
	for _, model in ipairs(droneFolder:GetChildren()) do
		...
		local color = hitRate.getTeamColor(relation)
...
local function buildTargetFromPlayer(player)
	...
	local part = character:FindFirstChild(hitRate.getAimPartName()) or character:FindFirstChild("HumanoidRootPart")
	if not part then
		return nil
	end
...
local function getActiveAimTarget()
	if sharedState.killAllForcedTarget then
		local part = sharedState.killAllForcedTarget.part
		if part and part.Parent then
			sharedState.killAllForcedTarget.position = part.Position
			return sharedState.killAllForcedTarget
		end
...
local function orbitAndKillPlayer(player)
	...
	while tick() - startTime < spinDuration do
		...
		sharedState.killAllForcedTarget = buildTargetFromPlayer(player)
		currentTarget = sharedState.killAllForcedTarget
		if not sharedState.killAllForcedTarget then
			break
		end
		...
		Camera.CFrame = CFrame.new(Camera.CFrame.Position, sharedState.killAllForcedTarget.position)
...
local function applyRapidFire(t2)
	...
	-- SettingsGun.FireRate is RPM ù never write interval seconds into it.
	if settings and tonumber(settings.FireRate) and settings.FireRate > 0 then
...
local function applyWalkSpeedBoost()
	...
	local speed = Constants.BOOST_WALK_SPEED
	if t2 and t2.States and t2.States.Sprinting and not t2.Aiming then
		speed = Constants.BOOST_SPRINT_SPEED
	end
...
local function installMovementHooks()
	...
	hookModuleMethod(movementModule, "ChangeMult", function(old, self, mult, ...)
		if Config.SpeedBoost then
			mult = (tonumber(mult) or 1) * (Constants.BOOST_WALK_SPEED / 17.9)
		end
	...
	if t2 then
		...
		if Config.ZoomBoost then
			...
			if t2.Aiming then
				...
				cam.FieldOfView = Constants.ZOOM_FOV
...
local function getShotAimPart(target)
	...
	if target.player then
		...
		if character then
			local partName = hitRate.getSilentAimPartName(character)
