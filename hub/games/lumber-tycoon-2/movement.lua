...
function M.create(opts)
	...
	local function restoreHumanoidStats(humanoid)
		if not humanoid or not humanoidDefaults then
			return
		end
	...
	local function applyFly()
		...
		local body = root:FindFirstChild("MicroHubLT2Fly")
		if not body then
			body = Instance.new("BodyVelocity")
			body.Name = "MicroHubLT2Fly"
		...
		local speed = math.clamp(tonumber(Config.FlySpeed) or 60, 10, Constants.MAX_FLY_SPEED)
